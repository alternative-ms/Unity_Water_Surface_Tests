...
Shader "Custom/Level5_Test2_VertexDisplacement_ColorRamp_Foam_PerlinWaves_Glitter_Oil"
{
    Properties
    {
        _MainTex ("_MainTex", 2D) = "white" {}
        _Glossiness ("_Glossiness", Range(0,1)) = 0.7
        _Metallic ("_Metallic", Range(0,1)) = 0.1

        [Header(Hexa Oil Layer)]
        _OilMaskTex ("_OilMaskTex", 2D) = "white" {}
        _OilGlossiness ("_OilGlossiness", Range(0,1)) = 0.5
        _OilMetallic ("_OilMetallic", Range(0,1)) = 0.7
        _OilBlinkSpeed ("_OilBlinkSpeed", Float) = 1.0

        [Header(Waves geometry)]
        _DispTex ("_DispTex (Grayscale)", 2D) = "gray" {}
        _Displacement ("_Displacement", Range(0, 10)) = 1.0
        _TexelSizeStep ("_TexelSizeStep", Range(0.0001, 0.1)) = 0.02
        ...
        _FoamTextureScale ("Foam Scale", Float) = 3.0
        _FoamThreshold ("Foam Threshold", Range(-1, 1)) = 0.5
        _FoamSpeed ("_FoamSpeed", Float) = 8.0
        _VelocityStrength ("Velocity Strength", Float) = 25.0

        [Header(Oil Glitter)]
        _GlitterThreshold ("Glitter Threshold", Range(0.5, 0.999)) = 0.95
        _GlitterDensity ("Glitter Density (Scale)", Float) = 300.0
        _GlitterBrightness ("Glitter Brightness", Range(0, 10)) = 1.0
    }
    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            void surf (Input IN, inout SurfaceOutputStandard o)
            {
                ...
                fixed oilMaskRaw = tex2D(_OilMaskTex, IN.uv_MainTex).r;
                fixed oilMask = 1 - lerp(oilMaskRaw * mainTexColor.a, 1.0, oilMaskRaw);

                oilMask = oilMask * (1-oilMaskRaw) * (1-oilMaskRaw);
                ...
                fixed3 waterFinalColor = mainTexColor.r * waterWithFoam;
                float3 waterEmission = foamColor * velocityMask * foamColor.a * mainTexColor.a * 5;
                ...
                float3 oilEmission = (mainTexColor.b * abs(frac(_Time.y * _OilBlinkSpeed) * 2.0 - 1.0) + float3(1.0, 1.0, 1.0) * glitterMask * _GlitterBrightness);

                o.Albedo = lerp(waterFinalColor, mainTexColor.g, oilMask);
                o.Metallic = lerp(_Metallic, _OilMetallic, oilMask);
                o.Smoothness = lerp(_Glossiness, _OilGlossiness, oilMask);
                o.Emission = lerp(waterEmission, oilEmission, oilMask);
                o.Alpha = 1;
            }
        ENDCG
    }
